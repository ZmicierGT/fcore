...
Please note that Fcore is a tool which helps you to easily implement and test your own financial strategies but it does not provide any 'out of the box' investment solutions. Consider all the provided strategies as programming examples which help you to implement your own strategies. Also it worth mentioning that this project is mostly focused on financial screening but not on creation of high-freqency trading systems. I believe that in the future the difference between these two approaches will become less significant but currently the goal of screening is to evaluate a large number of securities being not really concerned of speed. The same time HFT usually works with a limited number of securities (or a single one) and needs to process incoming data as fast as possible. However, if your strategy is not critically depending on speed, sure you may use this framework to build your autmated trading system.
