...
def test_non_existent_not_retried_across_instances(make_fmp, tmp_path):
    db = str(tmp_path / 'test.sqlite')

    inst1, _ = make_fmp(profile='profile_non_existent.json', db_name=db)
    inst1.db_connect()
    with pytest.raises(FdataError):
    ...
    inst2, fake = make_fmp(profile='profile_non_existent.json', db_name=db)
    inst2.db_connect()
    with pytest.raises(FdataError):
...
def test_non_existent_recovery_with_refetch(make_fmp, tmp_path):
    db = str(tmp_path / 'test.sqlite')
    ...
    inst2, _ = make_fmp(db_name=db, refetch=True)
    inst2.db_connect()
